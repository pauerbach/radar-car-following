...
datasets = {
    "Higher SNR": {
        "ttc": "./ttc_radar_evaluation_snr_6.npy",
        "headway": "./headway_radar_evaluation_snr_6.npy",
    },
    "Lower SNR": {
        "ttc": "./ttc_radar_evaluation_snr_1.npy",
        "headway": "./headway_radar_evaluation_snr_1.npy",
    },
    "Lower RCS": {
        "ttc": "./ttc_radar_evaluation_rcs_04.npy",
        "headway": "./headway_radar_evaluation_rcs_04.npy",
    },
    "Higher RCS": {
        "ttc": "./ttc_radar_evaluation_rcs_18.npy",
        "headway": "./headway_radar_evaluation_rcs_18.npy",
    },
    "No Reflections": {
        "ttc": "./ttc_radar_evaluation_no_groundreflections.npy",
        "headway": "./headway_radar_evaluation_no_groundreflections.npy",
    },
    "Baseline": {
        # "ttc": "./ttc_radar_new.npy",
        # "headway": "./headway_radar_new.npy",
        "ttc": "./ttc_radar_baseline.npy",
        "headway": "./headway_radar_baseline.npy",
    },
}
...
for name, paths in datasets.items():
    ...
    # Latex output
    print(f"--- {name} ---")
    print("TTC min, TTC P1, TTC Viol, Headway min, Headway P1, Headway Viol")
    print(f"{np.min(ttc_filtered):.2f}", end=" & ")
    print(f"{np.percentile(ttc_filtered, 1):.2f}", end=" & ")
    print(f"{ttc[ttc < TTC_THRESHOLD].shape[0] / NUMBER_OF_RUNS * 100:.2f}", end=" & ")
    print(f"{np.min(headway):.2f}", end=" & ")
    print(f"{np.percentile(headway, 1):.2f}", end=" & ")
    print(f"{headway[headway < HEADWAY_THRESHOLD].shape[0] / NUMBER_OF_RUNS * 100:.2f}")
    print()

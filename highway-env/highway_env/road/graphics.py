...
class RoadGraphics(object):
    ...
    @staticmethod
    def display_projected_vehicle(
        surface: WorldSurface, vehicle: Vehicle, road: Road
    ) -> None:
        ...
        for lane_id in side_lanes:
            ...
            projected_vehicle = Vehicle(vehicle.road, vehicle.position)
            projected_vehicle.LENGTH = vehicle.LENGTH
            projected_vehicle.WIDTH = vehicle.WIDTH
            ...
            VehicleGraphics.display(
                projected_vehicle, surface, transparent=True, label=False
            )
    ...
    def display_information(
        ...
        # IDM related information
        if vehicle.leader is not None:
            current_distance = vehicle.lane_distance_to(vehicle.leader)
            desired_distance = vehicle.desired_gap(vehicle, vehicle.leader)
            text = f"Current distance {current_distance:.2f} Desired distance {desired_distance:.2f}"
            position = [100, 900]
            font = pygame.font.Font(None, 50)
            text = font.render(text, 1, (10, 10, 10), (255, 255, 255))
            surface.blit(text, position)

        # MOBIL related information
        unsafe_braking_required = False
        jerk_left = jerk_right = 0
        for lane_index in road.network.side_lanes(vehicle.lane_index):
            # Is the candidate lane close enough?
            if not road.network.get_lane(lane_index).is_reachable_from(
                vehicle.position
            ):
                continue
            # Is the maneuver unsafe for the new following vehicle?
            new_preceding, new_following = vehicle.road.surrounding_vehicles(
                vehicle, lane_index
            )
            old_preceding, old_following = vehicle.road.surrounding_vehicles(vehicle)

            self_pred_a = vehicle.acceleration(
                ego_vehicle=vehicle, front_vehicle=new_preceding
            )
            new_following_a = vehicle.acceleration(
                ego_vehicle=new_following, front_vehicle=new_preceding
            )
            new_following_pred_a = vehicle.acceleration(
                ego_vehicle=new_following, front_vehicle=vehicle
            )

            # unsafe braking required?
            if new_following_pred_a < -vehicle.LANE_CHANGE_MAX_BRAKING_IMPOSED:
                unsafe_braking_required = True

            # Is there an acceleration advantage for me and/or my followers to change lane?
            self_a = vehicle.acceleration(
                ego_vehicle=vehicle, front_vehicle=old_preceding
            )

            old_following_a = vehicle.acceleration(
                ego_vehicle=old_following, front_vehicle=vehicle
            )
            old_following_pred_a = vehicle.acceleration(
            ...
            if lane_index == road.network.left_lane(vehicle.lane_index):
                jerk_left = jerk
            else:
                jerk_right = jerk

        text = f"Jerk left {jerk_left:.2f} jerk right{jerk_right:.2f}"
        if unsafe_braking_required:
            text = "Unsafe Braking required"
        position = [100, 940]
        font = pygame.font.Font(None, 50)
        text = font.render(text, 1, (10, 10, 10), (255, 255, 255))
        surface.blit(text, position)
